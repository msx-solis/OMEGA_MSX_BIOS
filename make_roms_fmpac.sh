...
dd if=/dev/zero ibs=1k count=32 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #16+16k
dd if=/dev/zero ibs=1k count=32 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin # solo normal 32k
#cat ${SYSTEM_ROMS_DIR}/knight.rom >> omega_msx2+_all_ntsc.bin #32k solo test audio
dd if=/dev/zero ibs=1k count=32 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #32k
dd if=/dev/zero ibs=1k count=16 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #16k
#cat ${SYSTEM_ROMS_DIR}/phc-70fd2_fmbasic.rom >> omega_msx2+_all_ntsc.bin #16k #desactivado en version Japo jp1=0
dd if=/dev/zero ibs=1k count=32 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #32k
# patch the keys
...
#cat ${SYSTEM_ROMS_DIR}/obsobios.rom >> omega_msx2+_all_ntsc.bin #16k #esto no va
dd if=/dev/zero ibs=1k count=16 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #16k
dd if=/dev/zero ibs=1k count=16 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin #16k
# patch the keys
if [ "$PATCH_KEYS" -eq "1" ]; then
