...
# to be compatible with all games
#!/bin/bash
...
cat ${SYSTEM_ROMS_DIR}/phc-35j_msx2psub.rom >> omega_msx2+_all_ntsc.bin
cat ${SYSTEM_ROMS_DIR}/phc-35j_kanjibasic.rom >> omega_msx2+_all_ntsc.bin
dd if=/dev/zero ibs=1k count=144 | tr "\000" "\377" >> omega_msx2+_all_ntsc.bin
# patch the keys
...
if [ "$PATCH_VERSION" -eq "1" ]; then
  dd if=rom_version_patch.bin of=omega_msx2+_all_ntsc.bin bs=1 seek=262187 conv=notrunc
fi
